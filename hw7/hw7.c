        ...
        for (int i = 0; i < cb.len; i++) {
            ...
            for (int t = 1; t <= p->vlen; t++) {
                ...
                if (kbhit() && getch() == ESC) {  // the user pressed ESC
                    cont = FALSE;           // breaks the outer loop
...
void CreateCharBuffer(CharBuffer* cb, char text[], int width, int height)
{
    ...
    cb->chars = (MovingCharacter*)malloc(sizeof(MovingCharacter) * cb->len);
    cb->index = (int*)malloc(sizeof(int) * cb->len);
...
void ShuffleIndex(CharBuffer* cb)
{
    // shuffle cb->index using the algorithm used in the previous homework
